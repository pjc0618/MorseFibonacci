...
fib		
    ...
    CMP R0, #1
    BGE Recurse
    MOV R0, #0 ; return value in eax
    B MorseDigit
	
Recurse:
    PUSH R1 ; preserve value of ebx
    SUB R0, #1
    PUSH R0
    BL fib
    MOV R1, R0 ; ebx is preserved by callee, so it is safe to use
    SUB SP, #1 ; decrement the value already on the stack
    BL fib
    ADD R0, R1 ; return value in eax
    ADD SP, #4 ; remove value from stack
    POP R1 ; restore old value of ebx
